...
function Input_Digit(digit) {
    ...
    // we are checking to see if Wait_Second _Operand is ture and set
...
function Handle_Operator(Next_Operator) {
    ...
    // displayed on the screen to a number and then stire the results in
    // Calculator.First_Operand if it doesnt already exist
    const Value_of_Input = parseFloat(Display_Value);
    //checks if an operator already exists and id Wait_Second_Operand is true, 
...
keys.addEventListener('click', (event) => {
    ...
    // that was clicked
    const { target } = event;
    // if the element that was clicked on is not a button, exit the dunction
